...
export default function Admin() {
  ...
  async function handleUpdate(data: ProjectInsert, imageFile?: File) {
    if (!editing) return;
    await updateProject(editing.id, data, imageFile);
    setEditing(null);
  }
    ...
    <main className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col gap-3">
        {projects.map(project => (
          <div
            key={project.id}
            className="flex flex-col items-center justify-between p-4 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <div>
              <div>
              ...
              <div className="flex gap-3">
                <button onClick={() => handleEditClick(project)} className="text-xs px-3 py-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                  Editar
                </button>
                <button
            ...
            </div>
            {
              editing?.id === project.id && (
                <div className="mt-2 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800">
                  <ProjectForm
                    initial={editing}
